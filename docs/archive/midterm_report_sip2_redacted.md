...
### 2.1対象ユーザーとアプリ概要
  ...
  ポモドーロタイマーとToDo管理をM5Stack上で完結させることで、作業中のアプリ切替・通知チェックといった作業中断要因を最小化できる。
...
|----------------|--------------------------|------|------|------|------|
| TOPページ（Web） | 機能ごとのページへの遷移、それらの紹介 | メンバーC | 完了 | 100%  | 各機能のページへの遷移ボタン/各機能の説明用ポップアップ（モーダル）の作成完了 |
| ポモドーロタイマー（Web） | 作業時間・休憩時間の登録 | メンバーC | 進行中 | 80%  | 作業時間・休憩時間の登録/過去設定の削除機能は完成、再設定ボタン未完成 |
| ToDoリスト（Web） | タスクの名前と期限日、カテゴリの設定 | メンバーC | 進行中 | 80%  | タスク名・期限日・カテゴリの登録/完了・未完了の変更/完了タスクの削除完成、カテゴリごとに分けて表示する機能未完成 |
...
| ToDoリスト（M5Stack）| Flask APIから取得したToDoリストから内容と期限を表示し、達成の可否を行う | メンバーE | 進行中 | 70% | Flask APIとの通信機能が未完成 |
| UIデザイン（M5Stack）| M5Stackの各アクション時における視覚的デザイン| メンバーE | 進行中 | 80% | ポモドーロタイマーのworkとbreakの切り替え時やToDo達成時などの色覚的なデザインが未完成 |
