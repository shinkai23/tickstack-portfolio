...
* 得られる価値：スマートフォンからの通知・SNS・動画などによる注意の分散を防ぎ、ユーザーが「今やるべきこと」だけに意識を集中できる環境を提供する。 ポモドーロタイマーとタスク管理をM5Stack上で完結させることで、作業中のアプリ切替・通知チェックといった作業中断要因を最小化できる。
...
- メンバーD（サーバー開発）（サーバー開発）: Dockerによるチーム内の開発環境構築。M5Stack用ポモドーロタイマー設定を保存するデータベース設計・実装。フロントエンド・M5Stack間の連携コード作成。
- メンバーE（M5Stack開発）（M5Stack開発）
